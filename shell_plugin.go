...
func loadPluginFile(path string) (map[string]map[string]reflect.Value, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("open plugin %s error: %w", path, err)
	}

	exportSym, err := p.Lookup("Export")
	if err != nil {
		return nil, fmt.Errorf("symbol lookup error in plugin %s: %w", path, err)
	}

	export, ok := exportSym.(func() (string, map[string]reflect.Value))
	if !ok {
		return nil, fmt.Errorf("plugin %s has invalid Export signature", path)
	}
	...
	return nil, fmt.Errorf("empty symbols in plugin %s", path)
}

// ToDo: remove: now plugins are loaded at runtime, so loadPlugins in one go is no longer needed
// load .gp files
func loadPlugins(pluginDir string) error {
	...
	if err := filepath.WalkDir(pluginDir, func(path string, d os.DirEntry, walkErr error) error {
		if walkErr == nil && d.Type().IsRegular() && strings.HasSuffix(d.Name(), ".gp") {
			if _, err := loadPluginFile(path); err != nil {
